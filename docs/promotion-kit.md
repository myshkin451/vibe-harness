...
Copy, adapt, and post these when launching or sharing Vibe Harness.
...
```text
I kept seeing the same AI coding failure:

Session 1: magic.
Session 4: "wait, why did we choose this?"
...
It is intentionally not a heavy framework. The idea is to give agents a runway, not a cage.
```

## Reddit / Community Post

```text
I made a tiny skill for making AI-coded projects easier to continue after the first session.

Instead of a giant governance template, it creates the smallest useful harness:
- AGENTS.md: short map for coding agents
- docs/index.md: durable context router
...
- docs/index.md：长期上下文索引
- progress.md：状态板和验证证据
- 必要时才加 exec plan / decision record

目标不是治理，而是让 AI-coded 项目能持续。
```
...
Do not ask people to "star" first. Ask for real feedback:

```text
If you have a project where AI agents lost context after a few sessions, try this on a copy of the repo and tell me which artifact was useful or annoying.
```
